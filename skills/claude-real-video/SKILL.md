...
- The `[whisper]` extra is required for speech-to-text — pip never installs extras on its own. The first transcription then downloads a whisper base model (~139 MB). If `faster-whisper` is already installed, crv uses it automatically and the extra is unnecessary.
...
- **Windows: always run crv with `$env:PYTHONUTF8 = "1"`** — otherwise the cp1252 console crashes on the final "✓ Done" print and the grids (generated after that print) are never created. The suite installer sets `PYTHONUTF8=1` permanently.

---
Origin: https://github.com/HUANGCHIHHUNGLeo/claude-real-video (MIT) — skill adapted from the upstream project's own SKILL.md, with the Windows UTF-8 note and --speakers doc added.
